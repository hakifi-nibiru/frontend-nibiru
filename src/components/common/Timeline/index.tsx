...
const MOBILE_LINE = 1050;

export const Timeline: React.FC<TimelineProps> = ({ children, ...rest }) => {
	const [borderHeight, setBorderHeight] = useState(0);
	const NUMBER_OF_WALLETS = 1;
	const isMobile = useIsTablet();
	useEffect(() => {
		const handleScroll = () => {
			const scrollTop = window.scrollY || document.documentElement.scrollTop;
			const maxHeight = isMobile ? 2400 : 1850;
			const contentHeight = isMobile ? MOBILE_LINE + NUMBER_OF_WALLETS * 70 : 1746;
			let height = 0;
	...
	return (
		<div className="relative">
			...
			<div {...rest} className={cn(styles.timeline)}>
				{children}
			</div>
			<div
			...
			<div
				className={styles.line}
				style={{
					height: isMobile ? MOBILE_LINE + 1 * 70 : 1746,
...
export const TimelineItem: React.FC<TimelineItemProps> = (props) => {
	...
	useEffect(() => {
		const handleScroll = () => {
			const scrollTop = window.scrollY || document.documentElement.scrollTop;
			const maxHeight = isMobile ? 2500 - (NUMBER_OF_WALLETS + 1) * 100 : 1850;
			const contentHeight = isMobile ? 1485 : 1746;
			let height = 0;
			if (scrollTop - maxHeight > contentHeight) {
				height = contentHeight;
			} else if (scrollTop - maxHeight < 0) {
	...
	const renderImage = useCallback(() => {
		...
		return (
			<img
				...
				className={cn(styles.dot, "absolute size-5 dot")}
				alt={imageName}
			/>
		);
	...
	return (
		<div
			...
			{renderImage()}
			<div
				className={cn(styles.date, styles[side], {
			...
			{isButton === true ? null : (
				<div
					...
					<div>
						...
						<p
							className={cn("font-saira lg:text-base text-xs lg:mt-4 mt-1", {
								"text-typo-primary": inView,
